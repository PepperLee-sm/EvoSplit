...
if __name__=='__main__':
    ...
    if args.model=='esm1b':
        ...
        for name, inputs in msas.items():
            inputs = greedy_select(inputs, num_seqs=1024) # can change this to pass more/fewer sequences
            batch_labels, batch_strs, batch_tokens = batch_converter([inputs])
            batch_tokens = batch_tokens.to(next(mdl.parameters()).device)
            print('predicting...')
            pred = mdl.predict_contacts(batch_tokens)[0].cpu()
            pred_ = pred.clone().numpy()
            contacts_filtered, _ = map_top(pred.clone(), 15/2)
            top_contacts = contacts_filtered.numpy().copy()
            top_contacts[np.where(top_contacts>0)] = 1
            print(np.sum(pred_))
            np.savetxt("%s/%s_%s_%s.npy" % (args.o, args.model, args.keyword, name), pred_)
            print("wrote %s/%s_%s_%s.npy" % (args.o, args.model, args.keyword, name))
            # preds.append(pred)
            # names.append(name)

fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(4, 4), sharex=True, sharey=True)
ax.imshow(pred_, origin='lower', cmap='Blues')
ax.set_title("prob")
plt.savefig(f"{args.o}/{name}_prob.png")

fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(4, 4), sharex=True, sharey=True)
ax.imshow(top_contacts, origin='lower', cmap='Blues')
ax.set_title("contacts (top 15/2L)")
plt.savefig(f"{args.o}/{name}_contacts.png")