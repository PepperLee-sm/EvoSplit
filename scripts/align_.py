...
def align(pdb1: Structure, pdb2: Structure):
    ...
    # residue list for 2 objects
    refe_residue_list , mobi_residue_list = {}, {}
    for residue in reference.get_list():
        residue_id = residue.get_id()
        refe_residue_list[residue_id[1]] = residue
    for residue in mobile.get_list():
        residue_id = residue.get_id()
        mobi_residue_list[residue_id[1]] = residue

    # atom list for subsequent calculation
    ...
    for refe_res in mapping:
        for atom in backbone_atoms:
            ...
            if atom in refe_residue_list[refe_res] and atom in mobi_residue_list[mapping[refe_res]]:
                refe_atom_list.append(refe_residue_list[refe_res][atom])
                mobi_atom_list.append(mobi_residue_list[mapping[refe_res]][atom])
